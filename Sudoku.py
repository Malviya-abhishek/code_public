...
def solver(arr):                          # recursive solution of problem
    find = findZero(arr)                  # find zero and return them in to find tuple
    if find is None:                     # if none then we have find solution
        return True
    for k in range(1, 10):               # checking what can be filled
        if isValid(arr, k, find):         # is this move valid
            arr[find[0]][find[1]] = k     # filling that number
            if solver(arr):               # if this give solution then it okay
                return True
            arr[find[0]][find[1]] = 0     # otherwise try it again

    return False                         # return that no solution exit


def solverSpecial(position, key):       # driver function to check that a num at a position will give solution
    ...
    for i in range(cols):
        ...
        arr.append(col)                     # copying original matrix

    if isValid(arr, key, position):
...
def isValid(grid, num, pos):               # check that it is valid to put num at this pos
    row = pos[0]
    col = pos[1]
    ...
    for i in range(0, 3):
        for j in range(0, 3):
            if num == grid[i + row][j + col]:     # checking in it
                return False
            ...
            if event.type == pygame.KEYDOWN:            # any button is pressed
                ...
                if event.key == pygame.K_RETURN:                 # make an entry of key
                    if isItRed and key:                          # check valid cell is selected and a key is given
                        if solverSpecial(cellNum, key):   # solve the matrix
                            print(cellNum)
